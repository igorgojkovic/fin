...
iF tpdfprint<>'1'
   ...
   IF LEN(ALLTRIM(uslov))=0
      REPORT FORM &mime_reporta PREVIEW summary
   ELSE
      REPORT FORM &mime_reporta FOR &uslov PREVIEW summary
   endif   
ELSE
   *SET STEP ON 
   ...
   *myfile=mdata02+'\'+mfile
   ...
   IF LEN(ALLTRIM(uslov))=0   
      REPORT FORM &mime_reporta NOCONSOLE TO PRINTER summary
   ELSE
      REPORT FORM &mime_reporta NOCONSOLE TO printer FOR &uslov  summary  
   endif   
   * Cleanup
   lcpdf = .NULL.
   release lcpdf
