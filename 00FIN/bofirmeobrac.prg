...
MCENOVNIK2=CENOVNIK
manaziv=an0.naziv
replace sifra WITH STR(VAL(ALLTRIM(sifra)),5,0)
msifra=sifra
*SET STEP ON 
*-----------TABELA OBRACUNA------------
select VPOBRAC
...
IF PSMECE='D'
   ...
   IF mcenovnik2='T'
      REPLACE SMECE WITH (60*MCSMECE)*(100-POPSME)/100
      REPLACE SMECE60 WITH (POV-60)*MCSMECE60P*(100-POPSME)/100   
      replace pov60 WITH pov-60
   ELSE
      replace pov60 WITH 0
   endif   
ELSE
   REPLACE SMECE WITH 0
   REPLACE SMECE60 WITH 0
ENDIF
*------------OBRACUN ZA GREJANJE
IF PGRE='D'
...
SELECT vp
   ...
   IF IMARABAT=1
      MRABOSNOV=VODA+DOPV+KAN+DOPK+SMECE+SMECE60+CISCENJE+GREJANJE+DINOST1+DINOST2
   ELSE
      MRABOSNOV=0
   ENDIF      
   *MMAKSPOT=MMES*15
   ...
   *-----OBRACUN POREZA-------------------------------
   ...
   MUKUPNO=VODA+DOPV+KAN+DOPK+SMECE+SMECE60+CISCENJE+GREJANJE+DINOST1+DINOST2
   MUKUPNO2=VODA+DOPV+KAN+DOPK+SMECE+SMECE60+GREJANJE   
   ...
   *------porez     
   ...
   IF SMECE>0
      ...
      OTHERWISE
         mosn0=MOSN0+SMECE    
      ENDcase
      MPORSME60=ROUND((SMECE60)*PROCSME/100,2)
      DO CASE PROCSME
      case PROCSME=VAL(TOSTOPA)
         MPOREZ20=MPOREZ20+MPORSME60
         MOSN20=MOSN20+SMECE60
      case PROCSME=VAL(TNSTOPA)
         MPOREZ10=MPOREZ10+MPORSME60
         MOSN10=MOSN10+SMECE60
      OTHERWISE
         mosn0=MOSN0+SMECE60    
      ENDcase
   ELSE
   ...
   REPLACE PORSME60 WITH MPORSME60
   ...
   REPLACE SVEGA WITH UKUPNO-SOC-RABAT+PORVODA+PORKAN+PORSME+PORSME60+PORGRE+PORCIS+POROST1+POROST2

   MOSNOV=MUKUPNO-MSOC-mrabat+MPORVODA+MPORKAN+MPORSME+MPORSME60+MPORGRE+MPORCIS+MPOROST1+MPOROST2
...
*   REPLACE SOC WITH -MSOC
   ...
   DO WHILE.T.
      ...
      IF FOUND()
         ...
         REPLACE CSMECE60 WITH 0
         REPLACE CSMECE60p WITH 0                  
   ...
   *msporazum=sPORAzum
   ...
   REPLACE CSMECE60 WITH MCSMECE60
   REPLACE CSMECE60P WITH MCSMECE60P
   ...
   replace pov60 WITH mpov60
   replace sifra WITH msifra
   replace anaziv WITH manaziv
*   REPLACE SVEGA WITH MSVEGA
   MDDUG=VODA+DOPV+KAN+DOPK+smece+SMECE60+ciscenje+grejanje+PORVODA+PORKAN+PORSME+PORGRE+PORCIS+POROST1+POROST2-SOC-RABAT+KAMATA+DINOST1+DINOST2
   MKAMATA=KAMATA
*   SELECT VPUPL
*   SET ORDER TO 1
*   SEEK Mpib
