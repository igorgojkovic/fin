...
*-----------OBRACUN KAMATE---------------
   ...
   set order to 
   SELECT JP
   USE JPKAM IN 0 EXCLU
   SELECT JPKAM
   ZAP
   DO JPKAM WITH MPIB,MDATOD,MDATDO
   SELECT JPKAM
*--------PRENOS KAMATE------------------
   SET ORDER TO 1
   MKAMATA=0
...
*      REPLACE POT WITH &AKRAJ-&APOC
      MPOT=POT
   ELSE
      REPLACE POJP WITH MMES*MESPAUSAL
      MPOT=POT
   ENDIF
   IF POJP<0
      REPLACE POJP WITH 0
...
*------------OBRACUN ZA VODU------------------   
   IF POJP>0
      IF PVODA<>' '
         IF POPVODA<>0
         MSOCVODA=ROUND(POJP*MCVODA,2)
         ELSE
         MSOCVODA=0
         ENDIF
         REPLACE VODA WITH ROUND(POJP*MCVODA,2)
         REPLACE DOPV WITH ROUND(POJP*MCDOPV,2)
...
*------------OBRACUN ZA KANALIZACIJU
      IF PKANAL<>' '
         IF POPKANAL<>0
         MSOCKANAL=ROUND(POJP*MCKANAL,2)
         ELSE
         MSOCKANAL=0
         ENDIF
         REPLACE KANAL WITH ROUND(POJP*MCKANAL,2)
         REPLACE DOPK WITH ROUND(POJP*MCDOPK    ,2)
...
IF PSMECE<>' '
   IF POPSMECE<>0
   MSOCSMECE=ROUND(POVOBJ*MCSMECE*MMES,2)
   ELSE
   MSOCSMECE=0
   ENDIF
   REPLACE SMECE WITH ROUND(POVOBJ*MCSMECE*MMES,2)
...
IF PGRADJZEM<>' '
   REPLACE GRADJZEM WITH ROUND(POVOBJ*CENA_GZ*MMES*(100-POPGRADJ)/100,2)
...
IF PINVODRZ<>' '
   REPLACE INVODRZ WITH ROUND(MCINVODRZ*POVOBJ*MMES*(100-POPINVOD)/100,2)
...
*------------OBRACUN ZA OSTALO
IF POSTALO1<>' '
   REPLACE OSTALO1 WITH ROUND(MCOSTALO1*MMES,2)
...
IF POSTALO2<>' '
   REPLACE OSTALO2 WITH ROUND(MCOSTALO2,2)
...
IF LIFT<>0
   REPLACE PORLIFT WITH ROUND(18*LIFT/100,2)
ELSE
   REPLACE PORLIFT WITH 0   
ENDIF
MPORLIFT=PORLIFT
...
MPOR18=MPORVODA18+MPORKANAL18+MPORSMECE18+MPORGRADJ18+MPORCISCE18+MPORINVOD18+MPOROSTA118+MPOROSTA218+MPOROSTA318+MPORLIFT
MOSN18=MOSNVODA18+MOSNKANAL18+MOSNSMECE18+MOSNGRADJ18+MOSNCISCE18+MOSNINVOD18+MOSNOSTA118+MOSNOSTA218+MOSNOSTA318+LIFT
...
*-------OBRACUN RABATA-----------------   
   ...
   IF IMARABAT=1
      MRABOSNOV=VODA+DOPV+KANAL+DOPK
   ELSE
      MRABOSNOV=0
   ENDIF      
   IF MRABOSNOV>0
...
*----OBRACUN UKUPNO---------------------
   MUKUPNO=VODA+DOPV+KANAL+DOPK+SMECE+GRADJZEM+CISCENJE+INVODRZ+OSTALO1+OSTALO2+OSTALO3+LIFT
   REPLACE POREZ WITH PORVODA+PORKANAL+PORSMECE+PORGRADJ+PORCISCE+PORINVOD+POROSTA1+POROSTA2+POROSTA3+PORLIFT
   REPLACE UKUPNO WITH MUKUPNO   
   REPLACE SVEGA WITH UKUPNO+POREZ+KAMATA-RABAT+SOC
...
*   MKSSIFRA=KSSIFRA
   ...
   *-------CENE-----------------
   ...
   REPLACE CGRADJZEM WITH MCENAGZ
   ...
   *----------POREZ------------
   ...
   REPLACE PORLIFT WITH MPORLIFT
   REPLACE LIFT WITH MLIFT
...
*---------DODAVANJE POLJA-----------
   ...
   REPLACE DUG WITH MSVEGA+MZAOKRUZ
