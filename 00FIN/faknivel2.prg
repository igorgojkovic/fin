PUSH KEY CLEAR
SET EXACT OFF
SELECT FAK
MBRKAL=BRKAL
SET ORDER TO 1
LOCATE FOR BRKAL=MBRKAL
MDATDOK=FAKG.DATDOK
*----ODREDJIVANJE SLEDECE NIVELACIJE i naloga
SELECT NIV
SET ORDER TO 4
LOCATE FOR BRFAKN=MBRKAL
IF.NOT.FOUND()
SET ORDER TO 1
GO BOTTOM
MNOVI=VAL(SUBSTR(BRKAL,2,5))+1
MLEN=LEN(ALLTRIM(STR(MNOVI)))
MBRRAC='N'+REPLI('0',5-MLEN)+ALLTRIM(STR(MNOVI))
MNOVI=VAL(BRNAL)+1
MLEN=LEN(ALLTRIM(STR(MNOVI)))
MBRNAL=REPLI('0',6-MLEN)+ALLTRIM(STR(MNOVI))
*-----VRACA SE U KALKULACIJU NA PRVU STAVKU
SELECT FAK
SET ORDER TO 1
MKOL=0
MKOLI=0
MVELUL=0
MVELIZ=0

*-------PROLAZ KROZ FAKTURU

DO WHILE.NOT.EOF()
   IF BRKAL<>MBRKAL
      EXIT
   ENDIF
   *-------PRONALAZENJE ROBE---
   MRSIF=RSIF
   RR=RECNO()
   *----TRAZENJE ROBE U RACUNIMA
   SELECT FAK
   SET ORDER TO 2
   MKOLI=0
   MVELUL=0
   SEEK MRSIF
   IF FOUND()
      DO WHILE.NOT.EOF()
         IF RSIF<>MRSIF
            EXIT
         ENDIF
         IF FAKG.DATDOK<=MDATDOK
            IF RECNO()<>RR
               MKOLI=MKOLI+KOLI
            ENDIF
         ENDIF
         SKIP
      ENDDO
   ENDIF
   *----TRAZENJE ROBE U KAL
   SELECT KAL
   SET ORDER TO 2
   MKOL=0
   MVELIZ=0
   SEEK MRSIF
   IF FOUND()
      DO WHILE.NOT.EOF()
         IF RSIF<>MRSIF
            EXIT
         ENDIF
         IF KALG.DATDOK<=MDATDOK
            MKOL=MKOL+KOL
         ENDIF
         SKIP
      ENDDO
   ENDIF
   *-------POVRATAK NA RACUN
   SELECT FAK
   SET ORDER TO 1
   GOTO RR
   MSCENA=ROB.VELCENA
   IF TOBJEKAT='V'
      MSCENA=ROB.VELCENA
      MVELCENA=VELCENA
   ELSE
      MSCENA=ROB.MALCENA
      MVELCENA=MALCENA
   ENDIF
   MTARIFA=ROB.TARIFA
   MPROCPOR=ROB.PROCPOR
   MTARIFAS=ROB.TARIFAS
   MPROCPORS=ROB.PROCPORS
   MPLP=ROB.PLP
   MVRSTA=ROB.VRSTA
   MSADKOL=MKOL-MKOLI
   *-------PRENOS U NIVELACIJU
   SELECT NIV
   IF MSCENA<>MVELCENA.AND.MSCENA<>0.AND.((MKOL-MKOLI)<>0)
      APPEND BLANK
      REPLACE BRKAL WITH MBRRAC
      REPLACE RSIF WITH MRSIF
      REPLACE TARIFA WITH MTARIFA
      REPLACE PROCPOR WITH MPROCPOR
      REPLACE STARIFA WITH MTARIFA
      REPLACE SPROCPOR WITH MPROCPOR
      REPLACE KOL WITH MKOL-MKOLI
      REPLACE SCENA WITH MSCENA
      REPLACE SIZNOS WITH ROUND(KOL*SCENA,2)
      REPLACE MALCENA WITH MVELCENA
      REPLACE MALVRED WITH ROUND(KOL*MALCENA,2)
      REPLACE VELCENA WITH MVELCENA
      REPLACE VELVRED WITH ROUND(KOL*VELCENA,2)
      REPLACE DATDOK WITH MDATDOK
         IF DTOS(DATDOK)>=TVAZIOD
            replace tarifa with ROB.TARIFA
            replace procpor with ROB.PROCPOR
         ELSE
            replace tarifa with ROB.TARIFAS
            replace procpor with ROB.PROCPORS
         ENDIF        
      REPLACE BRNAL WITH MBRNAL
      REPLACE BRFAKN WITH MBRKAL

         DO NIVRAC
 
   ENDIF
   SELECT FAK
   SKIP
ENDDO
ELSE

   DO  FORM PORUKAU WITH ' VEÆ JE URADJENA NIVELACIJA PO RACUNU'
   SELECT NIV
   SET ORDER TO 1
   SELECT FAK
   LOCATE FOR BRKAL=MBRKAL

   SET ORDER TO 
ENDIF
SET EXACT ON
POP KEY