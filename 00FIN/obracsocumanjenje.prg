...
PUSH KEY CLEAR
   ...
   *SET STEP ON
   IF mbknacin<>'1'
      ...
      MMNEOPOREZP=ROUND((MNEOPOREZP*(maCASUK)/MCZAKON),2)
      mbkzastita=ROUND(mbkzastita*maCASUK/MCZAKON,2)
      mprocsoc=doppio+dopzdr+dopnez
      mgranica0=ROUND((mbkzastita-(mmneoporezP*mprocpor/100)),2)
      mgranica1=1-(mprocpor/100)-mprocsoc
      IF MGRANICA1<>0
      mgranicabruto=ROUND(mgranica0/mgranica1,2)
      ELSE
      mgranicabruto=0
      ENDIF
      replace bkumanj WITH 0
      IF bruto>mgranicabruto
          MBKUMANJ=ROUND(BRUTO*MBKPROC*maCASUK/MCZAKON/100,2) 
          IF bruto-mbkumanj<=mgranicabruto
             mbkumanj=BRUTO-MGRANICABRUTO
          endif  
      endif   
      REPLACE BKUMANJ WITH MBKUMANJ
      REPLACE BRUTO WITH BRUTO-MBKUMANJ

   else
      *//ovde racunam umanjenje novi nacin//
      MCASUMANJ=CASPRAZ+CASBOL+CASBOL2+CASPLAC+CASPLAC2+CASGOD+CASVV+CASSUS
      MBRUTOA=DINUK+STIM1+STIM2+STIM3+FIKSNA
      MBRUTO2B=DINPRAZ+DINBOL+DINBOL2+DINPLAC+DINPLAC2+DINGOD+DINVV+DINSUS
      MBRUTO2C=TOPLI+REGRES+TERENSKI+DOTACIJA+DINSUS
      mprocsoc=doppio+dopzdr+dopnez
      IF (casuk-mcasumanj)>mczakon
         macasuk=casuk-mcasumanj
      ELSE
         macasuk=(casuk-mcasumanj)
      endif      
      MBKUMANJ=0
      MMNEOPOREZP=ROUND(MNEOPOREZP,2)
      mosnov0=mbrutoa-mbruto2b
      mneoporez0=ROUND(mmneoporezp*macasuk/mczakon,2)
      mporez0=ROUND((mosnov0-mneoporez0)*mprocpor/100,2)
      mdop0=ROUND(mosnov0*mprocsoc,2)
      mneto0=mosnov0-mporez0-mdop0
      mzastita=ROUND(mbkzastita*macasuk/mczakon,2)
      **********************
      MKOEF=ROUND(MACASUK/MCZAKON,2)
      mgranica0=ROUND((mbkzastita*MKOEF)-(mmneoporezP*MKOEF*mprocpor/100),2)
      mgranica1=1-(mprocpor/100)-mprocsoc
      IF MGRANICA1<>0
         mgranicabruto=ROUND(mgranica0/mgranica1,2)
      ELSE
         mgranicabruto=0
      ENDIF
      IF MOSNOV0>MGRANICABRUTO
          MBKUMANJ=ROUND(MOSNOV0*MBKPROC/100,2) 
          IF mosnov0-mbkumanj<mgranicabruto
             mbkumanj=mosnov0-mgranicabruto
          endif   
      ENDIF
...
*      replace dopsocS with bruto*mdopprocF     
   ...
   CASE MISPLATA=4
      ...
      mosnov=ROUND(((BRUTO-POREZ-MMNEOPOREZP)*mkorekc),MDEC)
      
      REPLACE NETO WITH MOSNOV

      REPLACE NETOSVE WITH NETO+NETOPREV

      replace osnovica WITH bruto-MNEOPOREZ-POREZ
      mdopproc=doppio+dopzdr+dopnez
      mdopprocf=dopfpio+dopfzdr+dopfnez      
         replace dopsocr with ROUND((bruto-MNEOPOREZ-POREZ)*mdopproc,MDEC)
         replace dopsocf with ROUND((bruto-MNEOPOREZ-POREZ)*mdopprocf,MDEC)
         REPLACE OSNOVICA WITH MOSNOV
         REPLACE SKALA WITH 1        
      IF MDOPPROC<>0
         REPLACE DOPPR WITH ROUND(DOPSOCR*DOPPIO/MDOPPROC,MDEC)
