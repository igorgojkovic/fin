...
*------------OBRACUN ZA KANALIZACIJU
      IF PKANAL<>' '
         IF POPKANAL<>0
         MSOCKANAL=ROUND(POT*MCKANAL,2)
         ELSE
         MSOCKANAL=0
         ENDIF
         REPLACE KANAL WITH ROUND(POT*MCKANAL,2)
         REPLACE DOPK WITH ROUND(POT*MCDOPK,2)
...
IF PGRADJZEM<>' '
   REPLACE GRADJZEM WITH ROUND(CENA_GZ*MMES*(100-POPGRADJ)/100,2)
...
IF PCISCENJE<>' '
   REPLACE CISCENJE WITH ROUND(MCCISCENJE*POVOBJ*MMES*(100-POPCISCE)/100,2)
...
*----OBRACUN UKUPNO---------------------
   ...
   REPLACE SVEGA WITH UKUPNO-RABAT+SOC
