...
DO WHILE.NOT.EOF()
   ...
   MFVRSTA=FVRSTA
   mdatdok=datdok
   AATVISERVIS091.LBLKAL.CAPTION=' KALKULACIJE '+MBRKAL+' '+DTOC(DATDOK)
   MBRNAL=BRNAL
   MDATDOK=DATDOK
   SELECT KAL
   SEEK MBRKAL
...
DO WHILE.NOT.EOF()
   ...
   ENDIF
   SELECT KOCKA2
   AATVISERVIS091.LBLKAL.CAPTION=' NIVELACIJE '+MBRNAL+' '+DTOC(MDATDOK)
...
DO WHILE.NOT.EOF()
   ...
   ENDIF     
   SELECT FAKG   
   AATVISERVIS091.LBLKAL.CAPTION=' FAKTURE '+MBRKAL+' '+DTOC(DATDOK)
   SKIP
ENDDO   
SELECT FAK
SET RELATION TO
...
USE TMP0 IN 0 
SELECT TMP0
COPY TO &KKOCKA4 FOR AUTOMNAL<>' '.AND.MDOK=DOK
USE
...
*      REPLACE UPOT WITH MPOT
   ENDIF 
   SELECT KOCKA
   AATVISERVIS091.LBLKAL.CAPTION=' GLAVNA KNJIGA '+MBRNAL+' '+DTOC(MDATDOK)
   SKIP
ENDDO
SELECT KOCKA4
...
*      REPLACE UPOT WITH MUPOT
      ...
      SELECT KOCKA5
      AATVISERVIS091.LBLKAL.CAPTION=' IZRACUNAVANJE SALDA '+MBRNAL+' '+DTOC(MDATDOK)
      SKIP
   ENDDO   
   SELECT KOCKA5
   USE
...
GO TOP
   ...
   DO WHILE.NOT.EOF()
      ...
      REPLACE BRKAL WITH MBRKAL
      SELECT KOCKA
      AATVISERVIS091.LBLKAL.CAPTION=' PUNJENJE GLAVNE BAZE '+MBRNAL+' '+DTOC(MDATDOK)
      SKIP
   ENDDO   
SELECT NALIZV
USE
